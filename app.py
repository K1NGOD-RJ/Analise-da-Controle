...
with col_graf3:
    if not df_filtrado.empty:
        # Cria uma coluna de data numérica para ordenação
        df_filtrado['ANO_MES_NUM'] = df_filtrado['ANO ENTREGA'] * 100 + df_filtrado['MES ENTREGA']
        evolucao = df_filtrado.groupby(['MES_ANO', 'RESPONSAVEL'])['QTD'].sum().reset_index()
        evolucao = evolucao.sort_values('ANO_MES_NUM')
